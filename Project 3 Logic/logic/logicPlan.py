...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    """Add to KB everything that is discribed in the project"""
...
"""
HELP FUNCTIONS THAT ARE DISCRIBED IN THE PROJECT BEFORE QUESTION 6
"""
...
def func3(known_map, non_outer_walls, KB):

    for x, y in non_outer_walls:
        
        """Locations where there is and isn't provably a wall correspondingly"""
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        func1(t, all_coords, non_outer_wall_coords, known_map, sensorAxioms, allLegalSuccessorAxioms, fourBitPerceptRules, agent, KB)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        """Call the helper functions that are implemented above question 6"""
        possible_locations = []
        func1(t, all_coords, non_outer_wall_coords, known_map, SLAMSensorAxioms, SLAMSuccessorAxioms, numAdjWallsPerceptRules, agent, KB)
        func3(known_map, non_outer_wall_coords, KB)
        func2(t, non_outer_wall_coords, possible_locations, KB)
